...
image_tensor = image_tensor[1000:3000, :, :]
...
image_flatten = image_tensor.reshape((2000, 640*640))
print(image_flatten.shape)

label_train = label[:1500]
label_test = label[1500:]

label_train = label_train.reshape((1500, 1))
label_test = label_test.reshape((500, 1))

print(label_train.shape)
print(label_test.shape)

image_flatten_train = image_flatten[:1500, :]
image_flatten_test = image_flatten[1500:, :]
...
filename = "P:/MQP_data/" + str(configuration) + "/Results/5_6_split/" + str(subject) + "/finalized_model_fixed.sav"
...
plt.ylabel('True')
#plt.show()
plt.savefig("P:/MQP_data/" + str(configuration) + "/Results/5_6_split/" + str(subject) + "5_states_fixed.png")

print(confusion_matrix(y_test,y_pred))
print(classification_report(y_test,y_pred))
print(svclassifier.classes_)
np.save("P:/MQP_data/" + str(configuration) + "/Results/5_6_split/" + str(subject) + "/class_values_fixed", svclassifier.classes_)
